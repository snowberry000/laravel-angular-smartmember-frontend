...
app.config( function( $stateProvider, paginationTemplateProvider )
{
	$stateProvider
		.state( "admin", {
			templateUrl: "/templates/components/admin/admin.html",
...
app.controller( 'AdminController', function( $scope, $q, $rootScope, smModal, User, smSidebar, $timeout, $localStorage, $location, Restangular, RestangularV3 , $stateParams, $state, $http, toastr, $window, Upload )
{
	...
	$rootScope.BaseStateExists = function()
	{
		return $rootScope.last_base_state.state;
	}
	...
	// 			smModal.Show( 'public.administrate.wizard', {
	...
	$scope.Init = function()
	{
		...
		if( $localStorage.user && $localStorage.user.id )
		{
			...
			$rootScope.user = RestangularV3.one( 'user', $localStorage.user.id ).get().then( function( response )
			{
	...
	$rootScope.isSitelessPage = function( specific_site )
	{
		...
		else if( subdomain == 'app' || subdomain == 'www' )
		{
			return true;
		}
