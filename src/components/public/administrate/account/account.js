...
app.config(function($stateProvider){
	$stateProvider
		.state("public.administrate.account",{
			url: "/account",
			templateUrl: "/templates/components/public/administrate/account/account.html",
...
app.controller('AccountController', function ($scope,$rootScope, $state, $location, $localStorage ,Restangular, toastr) {
	...
	if ($location.search().verification_hash)
	{
	        Restangular.one('linkedAccount/merge').customPOST({'verification_hash' :$location.search().verification_hash}).then(function(response){
	            ...
	            $state.go("public.administrate.account.settings", {}, {reload: true});
	        })
	}
