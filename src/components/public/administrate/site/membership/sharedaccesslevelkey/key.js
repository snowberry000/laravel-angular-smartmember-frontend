...
app.config(function($stateProvider){
	$stateProvider
		.state("public.administrate.site.membership.sharedaccesslevelkey",{
			url: "/sharekey/:id?",
			templateUrl: "/templates/components/public/administrate/site/membership/sharedaccesslevelkey/key.html",
...
app.controller("ShareAccessLevelKeyController", function ($scope, $q, $stateParams,smModal, $localStorage, $rootScope,  Restangular,toastr,$state) {
	...
	$scope.update = function(){
		$scope.key.put().then(function(response){
			...
			smModal.Show("public.administrate.site.membership.sharedaccesslevelkeys");
		})
	}
