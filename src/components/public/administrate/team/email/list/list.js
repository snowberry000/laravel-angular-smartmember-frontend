...
app.config(function($stateProvider){
	$stateProvider
		.state("public.administrate.team.email.list",{
			url: "/list/:id?",
			templateUrl: "/templates/components/public/administrate/team/email/list/list.html",
...
app.controller("smartMailListController", function ($scope,smModal,$rootScope, $stateParams, $localStorage, Restangular, toastr, $state) {
	...
	$scope.saveAndImport = function(){
	    ...
	    if ($scope.emailList.id){
	        ...
	        smModal.Show("public.administrate.team.email.import",{id:$scope.emailList.id});
	        return;
	    }
	    ...
	    Restangular.service("emailList").post($scope.emailList).then(function(response){
	        ...
	        smModal.Show("public.administrate.team.email.import",{id:response.id});
	    });
	...
	$scope.update = function(){
	    ...
	    $scope.emailList.put().then(function(response){
	        ...
	        toastr.success("List updated!");
	        smModal.Show("public.administrate.team.email.lists");
	    })
	}
