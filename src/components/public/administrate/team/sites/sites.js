...
app.controller( "SitesController", function( $scope, $rootScope, $filter, $localStorage, toastr, $location, Restangular, $state, notify )
{
	...
	$scope.revoke = function( $argSite )
	{
		...
		Restangular.all( 'siteRole/removeUserFromSite' ).customPOST( {
			'site_id': $argSite.id,
			'user_id': $rootScope.site.user_id
		} ).then( function( response )
		{
			$scope.sites_to_show = _.filter( $scope.sites_to_show, function( $tempSite )
			{
				return $tempSite.id != $argSite.id;
			} );
