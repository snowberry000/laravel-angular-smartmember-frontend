...
{
	$stateProvider
		.state( "public.admin.team.helpdesk.tickets.solved", {
			url: "/solved",
			templateUrl: "/templates/components/public/admin/team/helpdesk/tickets/solved/solved.html",
	...
	$scope.updateMultiple = function( property, value )
	{
		...
		Restangular.all( 'supportTicket' ).customPUT( data, 'bulk' ).then( function( response )
		{
			if( response.success )
			{
				...
				{
					$scope.filtered_tickets = _.reject( $scope.filtered_tickets, function( item )
					{
						return $scope.$parent.selection.selectedTickets.indexOf( item.id ) >= 0
					} );
					$scope.tickets = _.reject( $scope.tickets, function( item )
					{
						return $scope.$parent.selection.selectedTickets.indexOf( item.id ) >= 0
					} );
	...
	$scope.selectAll = function()
	{
		...
		if( $scope.all_selected )
		{
			$scope.$parent.selection = {
				ticketSelected: true,
				selectedTickets: _.pluck( $scope.filtered_tickets, 'id' )
			}
		}
		else
		{
