...
app.controller( "MenusController", function( $rootScope, $scope, $filter, smModal, $document, $localStorage, $location, $stateParams, Restangular, toastr )
{
	...
	$scope.open = function( next_item, menu )
	{
		...
		if( menu == 'footer' )
		{
			smModal.Show( "public.admin.site.appearance.footerMenuItem", { id: next_item.id, modal_options: { allowMultiple: true } } );
		}
		else
		{
			smModal.Show( "public.admin.site.appearance.menu", { id: next_item.id, modal_options: { allowMultiple: true } } );
		}
	};
