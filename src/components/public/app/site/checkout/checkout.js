...
{
	$stateProvider
		.state( "public.app.site.checkout", {
			url: "/checkout/:id",
			templateUrl: "/templates/components/public/app/site/checkout/checkout.html",
...
app.controller( 'CheckoutController', function( $scope, $site, $rootScope, $location, notify, $localStorage, $stateParams, Restangular )
{
	...
	$scope.initialize = function()
	{
		...
		$scope.paypal = _.findWhere( $scope.site.configured_app, {
			type: 'paypal',
        ...
        if( !$scope.paypal )
        {
            $scope.paypal = _.findWhere( $scope.site.configured_app, {
                type: 'paypal',
                company_id: $scope.site.company_id,
                default: "1"
            } );
        }
		...
		if( !$scope.paypal )
        {
            $scope.paypal = _.findWhere( $scope.site.configured_app, {
                    type: 'paypal',
                    site_id: $scope.site.id
                } ) || _.findWhere( $scope.site.configured_app, { type: 'paypal' } );
        }

        $scope.stripe = _.findWhere( $scope.site.configured_app, {
            type: 'stripe',
			site_id: $scope.site.id,
			default: "1"
		} );
		...
		if( !$scope.stripe )
        {
            $scope.stripe = _.findWhere( $scope.site.configured_app, {
                    type: 'stripe',
                    site_id: $scope.site.id
                } ) || _.findWhere( $scope.site.configured_app, { type: 'stripe' } );
        }
		...
		handler = StripeCheckout.configure( {
			...
			{
				Restangular.all( 'transaction' ).post( {
					access_id: $scope.access_level.id,
					token: token.id,
					type: 'stripe',
					name: name,
					email: token.email
	...
	$scope.init = function()
	{
		Restangular.one( 'accessLevel', $stateParams.id ).get().then( function( response )
		{
            ...
            if( typeof $scope.access_level.payment_methods != 'undefined' )
			{
				...
				{
					var new_stripe = _.findWhere( $scope.site.configured_app, {
							type: 'stripe',
							id: $scope.access_level.stripe_integration
						} ) || _.findWhere( $scope.site.configured_app, {
							type: 'stripe',
							id: $scope.access_level.stripe_integration + ''
						} )
				...
				{
					var new_paypal = _.findWhere( $scope.site.configured_app, {
							type: 'paypal',
							id: $scope.access_level.paypal_integration
						} ) || _.findWhere( $scope.site.configured_app, {
							type: 'paypal',
							id: $scope.access_level.paypal_integration + ''
						} )
