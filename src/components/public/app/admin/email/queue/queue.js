...
{
	$stateProvider
		.state( "public.app.admin.email.queue", {
			url: "/queue",
			templateUrl: "/templates/components/public/app/admin/email/queue/queue.html",
...
app.controller( "EmailQueueController", function( $scope,smModal,$rootScope, $localStorage, $location, $state, Restangular, toastr )
{
	...
	{
		$state.go('public.app.admin.email.sendmail', { 'job': job });
	}
} );