...
{
	$stateProvider
		.state( "public.app.admin.appearance.menus", {
			url: "/menus",
...
app.controller( "MenusController", function( $rootScope, $scope, $filter, smModal, $state , $document, $localStorage, $location, $stateParams, Restangular, toastr )
{
	$scope.sales_option = {};
	$site = $rootScope.site;
	$menus = null;
	...
	$scope.open = function( next_item, menu )
	{
		...
		if( menu == 'footer' )
		{
			$state.go( "public.app.admin.appearance.footerMenuItem", { id: next_item.id, modal_options: { allowMultiple: true } } );
		}
		else
		{
			$state.go( "public.app.admin.appearance.menu", { id: next_item.id, modal_options: { allowMultiple: true } } );
		}
	};
